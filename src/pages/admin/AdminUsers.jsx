...
export default function AdminUsers() {
    ...
    useEffect(() => {
        ...
        const newTotalPages = Math.max(1, Math.ceil(filteredUsers.length / currentItemsPerPage));
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden transition-all hover:shadow-lg relative">
            <div className="p-4">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                    ...
                    <p><strong>Sex:</strong> <span className="capitalize">{user.sexe ?? 'N/A'}</span></p>
                    <p><strong>Blood Group:</strong> {user.groupage ?? 'N/A'}</p>
                </div>
            </div>
                    ...
                    <div className="p-4 text-center">
                        ...
                        <div className="flex space-x-3 justify-center">
                            <button
                                ...
                                className={`bg-red-600 hover:bg-red-700 text-white text-xs py-2 px-4 rounded ${deletingUserId === user.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                            ...
                            <button
                                onClick={(e) => toggleDeleteConfirm(e, user.id)}
                                disabled={deletingUserId === user.id}
                                className={`bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs py-2 px-4 rounded hover:bg-gray-300 dark:hover:bg-gray-600 ${deletingUserId === user.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                                Cancel
                            </button>
                        </div>
    ...
    const Pagination = () => {
        ...
        const lastItemIndex = Math.min(currentPage * currentItemsPerPage, filteredUsers.length);
        ...
        return (
            <div className="px-3 sm:px-6 py-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between rounded-b-xl shadow-md">
                <div className="mb-3 sm:mb-0 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                    Showing {filteredUsers.length > 0 ? firstItemIndex : 0} to {lastItemIndex} of {filteredUsers.length} users
                </div>
                <div className="flex space-x-1.5">
                    ...
                    {pageNumbers.map((number) => (
                        <button
                            ...
                            className={`px-3 py-1 rounded-md text-xs sm:text-sm font-medium transition-colors ${currentPage === number
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                        <div className="relative">
                            ...
                            >
                                {viewMode === "table" ? (
                                    <>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-layout-grid mr-2"><rect width="7" height="7" x="3" y="3" rx="1" /><rect width="7" height="7" x="14" y="3" rx="1" /><rect width="7" height="7" x="14" y="14" rx="1" /><rect width="7" height="7" x="3" y="14" rx="1" /></svg>
                                        Card View
                                    </>
                                ) : (
                                    <>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-layout-list mr-2"><rect width="7" height="6" x="3" y="3" rx="1" /><path d="M14 6H21" /><rect width="7" height="6" x="3" y="15" rx="1" /><path d="M14 18H21" /></svg>
                                        Table View
                                    </>
                                )}
                            </button>
                                                    ...
                                                    <div className="mt-3 p-2 bg-red-50 dark:bg-red-900/20 rounded-md">
                                                        ...
                                                        <div className="flex space-x-2">
                                                            <button
                                                                onClick={() => handleDeleteUser(user.id)}
                                                                disabled={deletingUserId === user.id}
                                                                className={`flex-1 bg-red-600 hover:bg-red-700 text-white text-xs py-1 rounded ${deletingUserId === user.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                            >
                                                                {deletingUserId === user.id ? (
