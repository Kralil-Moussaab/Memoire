...
import { listAdminDoctors, getDoctorAdminStats, approveDoctor, deletDoctor } from "../../services/api";
...
export default function AdminDoctors() {
  ...
  const Pagination = () => {
    const currentItemsPerPage = viewMode === 'table' ? ITEMS_PER_PAGE_TABLE : ITEMS_PER_PAGE_GRID;
    const firstItemIndex = (currentPage - 1) * currentItemsPerPage + 1;
    const lastItemIndex = Math.min(currentPage * currentItemsPerPage, doctors.length);
    ...
    return (
      <div className="px-3 sm:px-6 py-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between rounded-b-xl shadow-md">
        <div className="mb-3 sm:mb-0 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
          Showing {doctors.length > 0 ? firstItemIndex : 0} to {lastItemIndex} of {doctors.length} doctors
        </div>
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6">
      <Message type={messageType} message={message} onClose={() => setMessage(null)} />
              ...
              {showSpecialtyDropdown && (
                <div className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg z-50 border dark:border-gray-700">
                  <div className="py-1 max-h-56 overflow-y-auto">
                    ...
                    {["Dentist", "Cardiologist", "Neurologist", "Dermatologist", "Orthopedic", "Gynecologist", "Generalist"].map((specialty) => (
                      <button
                        key={specialty}
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <div className="flex items-center justify-end space-x-1">
                        {doctor.approved === 0 && (
                          <button
                            ...
                            disabled={approvingDoctorId === doctor.id || deletingDoctorId === doctor.id}
                            className={`text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300 p-1 rounded-full hover:bg-green-50 dark:hover:bg-green-900/20 ${approvingDoctorId === doctor.id || deletingDoctorId === doctor.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                        ...
                        <button
                          ...
                          disabled={approvingDoctorId === doctor.id || deletingDoctorId === doctor.id}
                          className={`text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 p-1 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20 ${approvingDoctorId === doctor.id || deletingDoctorId === doctor.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                          title={deletingDoctorId === doctor.id ? 'Deleting...' : 'Delete Doctor'}
                        >
                        ...
                        <div className="mt-2 p-2 bg-red-50 dark:bg-red-900/20 rounded-md text-left">
                          ...
                          <div className="flex space-x-2">
                            <button
                              ...
                              className={`flex-1 bg-red-600 hover:bg-red-700 text-white text-xs py-1 rounded ${deletingDoctorId === doctor.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                              {deletingDoctorId === doctor.id ? (
                            ...
                            <button
                              onClick={(e) => toggleDeleteConfirm(e, doctor.id)}
                              disabled={deletingDoctorId === doctor.id}
                              className={`flex-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs py-1 rounded ${deletingDoctorId === doctor.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                              No
                            </button>
                          </div>
                ...
                <div className="p-4 md:p-6">
                  ...
                  <div className="mt-3 sm:mt-4">
                    <button
                      onClick={(e) => toggleDeleteConfirm(e, doctor.id)}
                      disabled={approvingDoctorId === doctor.id || deletingDoctorId === doctor.id}
                      className={`w-full bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400 py-2 rounded-lg transition-colors text-sm ${approvingDoctorId === doctor.id || deletingDoctorId === doctor.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      {deletingDoctorId === doctor.id ? (
